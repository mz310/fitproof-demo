...
def dashboard():
    ...
    active_session = WorkoutSession.query.filter_by(user_id=current_user.id, is_active=True).first()
    ...
    return render_template(
        "dashboard.html", active_session=active_session, recent_sessions=recent_sessions
    )
...
@login_required
def statistics():
    """Хэрэглэгчийн статистик хуудас"""
    from app.statistics import WorkoutStatistics, calculate_level

    user_stats = WorkoutStatistics(current_user.id)
    stats = user_stats.get_summary()
    level = calculate_level(current_user.xp_points)

    return render_template("statistics.html", stats=stats, level=level)
...
def start_session():
    ...
    # Шинэ сесс үүсгэх
    session = WorkoutSession(user_id=current_user.id, equipment_id=equipment.id)
...
def get_active_session():
    """Идэвхтэй сесс авах"""
    session = WorkoutSession.query.filter_by(user_id=current_user.id, is_active=True).first()
...
def user_stats():
    ...
    total_duration = (
        db.session.query(db.func.sum(WorkoutSession.duration_minutes))
...
def api_leaderboard():
    ...
    return (
        jsonify(
            {
                "leaderboard": [
                    {"rank": i + 1, "username": u.username, "xp_points": u.xp_points}
...
# ==================== ACHIEVEMENTS (Pair Programming Feature) ====================
...
def achievements():
    """
    Амжилтуудын хуудас
    Лаборатори 6: Pair Programming-д зориулсан feature
    """
    from app.achievements import AchievementService

    service = AchievementService(current_user.id)
    earned = service.get_earned_achievements()
    available = service.get_available_achievements()
...
@main_bp.route("/api/achievements/check", methods=["POST"])
@login_required
def check_achievements():
    """Шинэ амжилт авсан эсэхийг шалгах"""
    from app.achievements import AchievementService

    service = AchievementService(current_user.id)
    new_achievements = service.check_and_award_achievements()
